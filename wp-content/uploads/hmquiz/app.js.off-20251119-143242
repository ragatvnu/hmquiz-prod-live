...
(() => {
  ...
  const passCountOverrides = parseList(
    qs.get('pass_target') ||
    qs.get('passcount') ||
    qs.get('pass_count') ||
    qs.get('min_correct')
  ).map((token) => toPositiveInt(token));
  const getPassCountOverride = (levelIndex) => {
    if (!passCountOverrides.length) return null;
    const specific = passCountOverrides[levelIndex];
    if (typeof specific === 'number' && specific > 0) {
      return specific;
    }
    const fallback = passCountOverrides.find((num) => typeof num === 'number' && num > 0);
  ...
  function addEmailBox(container, stats) {
    if (!container) return;
    const isModalContext = !!(container?.closest && container.closest('.hmqz-modal'));
    const storedEmail = (() => {
      try { return localStorage.getItem('hmqz_email') || ''; } catch(e) { return ''; }
    })();
    const group = el("div", { className: "hmqz-emailbox" });
    if (isModalContext) {
      group.classList.add('hmqz-emailbox-inline');
    }
    group.innerHTML = `
    ...
    const renderPrintSheet = (info) => {
      ...
      const passTargetDetails = levelHistory
        .map((entry, idx) => {
          const raw = Number(entry.pass_target);
          if (!Number.isFinite(raw) || raw <= 0) return null;
          return { level: entry.level ?? (idx + 1), target: Math.round(raw) };
        })
        .filter(Boolean);
      const passRuleText = (() => {
        if (passTargetDetails.length === 1) {
          return `Need ≥ ${passTargetDetails[0].target} correct`;
        }
        if (passTargetDetails.length > 1) {
          const uniqueTargets = Array.from(new Set(passTargetDetails.map((d) => d.target)));
          if (uniqueTargets.length === 1) {
            return `Need ≥ ${uniqueTargets[0]} correct per level`;
          }
          return passTargetDetails
            .map((detail) => `L${detail.level}: ${detail.target}`)
            .join(' · ');
        }
        const fallbackPerLevel = Math.max(1, Number(stats.rules?.per_level) || 5);
        const fallbackRatio = clamp01(stats.rules?.pass_ratio ?? 0.8);
        const fallbackTarget = Math.max(1, Math.ceil(fallbackPerLevel * fallbackRatio));
        return `Need ≥ ${fallbackTarget} correct`;
      })();
      ...
      summary.append(
        ...
        metric('Pass target', passRuleText)
      );
  ...
  function renderLegacyTimer(root, payload) {
    ...
    function playLevel(levelIdx) {
      ...
      const levelTargetOverride = toPositiveInt(
        level?.pass_target ?? level?.pass_count ?? level?.min_correct
      );
      const urlTargetOverride = getPassCountOverride(levelIdx);
      const levelPassRatio = clamp01(level?.pass_ratio ?? level?.passRatio, passThreshold);
      const derivedTarget = levelTargetOverride ?? urlTargetOverride;
      const fallbackTarget = Math.ceil(perLevelTarget * levelPassRatio);
      const passCountTarget = Math.min(
        questions.length,
        Math.max(1, derivedTarget ?? fallbackTarget)
      );
      const answeredLog = [];
      let qIndex = 0;
      let correct = 0;
